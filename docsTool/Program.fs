...
module GenerateDocs =
    ...
    let renderWithMasterAndWrite masterCfg (outPath: FileInfo) navBar titletext bodytext pageSource =
        let contents =
            renderWithMasterTemplate masterCfg navBar titletext bodytext pageSource

        IO.Directory.CreateDirectory(outPath.DirectoryName)
        |> ignore
    ...
    let generateNav (cfg: Configuration) (generatedDocs: GeneratedDoc list) =
        ...
        let pages =
            ...
            |> List.filter
                (fun f ->
                    f.FullName.StartsWith(docsDir </> "content")
                    |> not)
            |> List.filter (fun f -> f.FullName.StartsWith(docsDir </> "files") |> not)
            |> List.filter
                (fun f ->
                    f.FullName.StartsWith(docsDir </> "index.html")
                    |> not)
                ...
                let pageSource =
                    ...
                    |> Option.map
                        (fun sp ->
                            sp
                                .FullName
                                .Replace(cfg.RepositoryRoot.FullName, "")
                                .Replace("\\", "/"))
    ...
    let copyAssets (cfg: Configuration) =
        Shell.copyDir
            (cfg.DocsOutputDirectory.FullName </> "content")
            (cfg.DocsSourceDirectory.FullName </> "content")
            (fun _ -> true)

        Shell.copyDir
            (cfg.DocsOutputDirectory.FullName </> "files")
    ...
    let regexReplace (cfg: Configuration) source =
        ...
        (source, replacements)
        ||> List.fold
                (fun state (pattern, replacement) -> Text.RegularExpressions.Regex.Replace(state, pattern, replacement))

    let stringContainsInsenstive (filter: string) (textToSearch: string) =
        textToSearch.IndexOf(filter, StringComparison.CurrentCultureIgnoreCase)
        >= 0
    ...
    let generateDocs (docSourcePaths: IGlobbingPattern) (cfg: Configuration) =
        let parse (fileName: string) source =
            let doc =
                ...
                let compilerOptions =
                    [| yield "--targetprofile:netcore" //"--targetprofile:netstandard"
                       yield! dlls |]
        ...
        let format (doc: LiterateDocument) =
            if not <| Seq.isEmpty doc.Errors then
        ...
        |> Seq.map
            (fun filePath ->
                ...
                let fs = file |> parse filePath |> format
                let contents = [ div [] [ fs |> RawText ] ]
    ...
    let generateAPI (cfg: Configuration) =
        ...
        let generate (projInfo: string) =
            ...
            let projName =
                IO.Path.GetFileNameWithoutExtension(projInfo)

            let targetApiDir =
                docsApiDir cfg.DocsOutputDirectory.FullName
                @@ projName

            let projDll =
                cfg.PublishPath.FullName
            ...
            let fi =
                FileInfo
                <| targetApiDir
                   @@ (sprintf "%s.html" generatorOutput.AssemblyGroup.Name)
                ...
                |> List.map
                    (fun m ->
                        let fi =
                            FileInfo
                            <| targetApiDir
                               @@ (sprintf "%s.html" m.Module.UrlName)
            ...
            let typeDocs =
                ...
                |> List.map
                    (fun m ->
                        let fi =
                            FileInfo
                            <| targetApiDir @@ (sprintf "%s.html" m.Type.UrlName)
    ...
    let renderDocs (cfg: Configuration) =
        ...
        let generateAPI = async { return generateAPI cfg }
    ...
    let buildDocs (cfg: Configuration) =
        renderDocs cfg |> renderGeneratedDocs false cfg
                    ...
                    |> Seq.iter
                        (fun m ->
                            ...
                            |> List.filter
                                (fun x ->
                                    generated
                                    |> List.exists (fun y -> y.OutputPath = x.OutputPath)
                                    |> not)
...
let main argv =
    try
            ...
            let config =
                ...
                ||> List.fold
                        (fun state next ->
                            ...
                            | BuildArgs.DocsOutputDirectory outdir ->
                                { state with
                                      DocsOutputDirectory = IO.DirectoryInfo outdir }
                            | BuildArgs.DocsSourceDirectory srcdir ->
                                { state with
                                      DocsSourceDirectory = IO.DirectoryInfo srcdir }
        ...
        | Watch args ->
            let config =
                ...
                ||> List.fold
                        (fun state next ->
                            ...
                            | WatchArgs.DocsSourceDirectory srcdir ->
                                { state with
                                      DocsSourceDirectory = IO.DirectoryInfo srcdir }
            ...
            use ds =
                GenerateDocs.watchDocs refreshWebpageEvent config
