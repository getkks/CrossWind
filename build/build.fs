...
let productName = "CrossWind"

let sln =
    __SOURCE_DIRECTORY__ </> ".." </> "CrossWind.sln"


let srcCodeGlob =
    !!(__SOURCE_DIRECTORY__ </> ".." </> "Source/**/*.fs")
...
let distDir = __SOURCE_DIRECTORY__ </> ".." </> "Output"
...
module DocsTool =
    ...
    let watch () =
        dotnet.watch
            (fun args -> { args with WorkingDirectory = docsToolDir })
            "run"
...
let generateAssemblyInfo _ =
    ...
    let (|Fsproj|Csproj|Vbproj|) (projFileName: string) =
        ...
        | f when f.EndsWith("fsproj", StringComparison.InvariantCultureIgnoreCase) -> Fsproj
        | f when f.EndsWith("csproj", StringComparison.InvariantCultureIgnoreCase) -> Csproj
        | f when f.EndsWith("vbproj", StringComparison.InvariantCultureIgnoreCase) -> Vbproj
